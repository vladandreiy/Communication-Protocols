...
void run_client(int sockfd) {
    ...
    while (read(STDIN_FILENO, buf, BUFLEN - 1) > 0 && !isspace(buf[0])) {
        ...
        bytes_send = write(sockfd, buf, sizeof(buf));
        memset(buf, 0, BUFLEN);
       
        // TODO 5: Receptionati un mesaj venit de la server
        bytes_received = read(sockfd, buf, sizeof(buf));
        fprintf(stderr, "Received: %s", buf);

        DIE(bytes_send != bytes_received, "transmission error");
...
int main(int argc, char* argv[])
{
    ...
    DIE(c != 0, "cannot connect");
    run_client(sockfd);

    // TODO 6: Inchideti conexiunea si socketul creat
    DIE(shutdown(sockfd, SHUT_RDWR) != 0, "error shutting down connection");
    close(sockfd);
    return 0;
}
