...
void ping(int sockfd, struct in_addr ip_addr, int count)
{
	...
	do {
		...
		icmp_hdr->type = 8;
		icmp_hdr->checksum = 0;
		icmp_hdr->un.echo.sequence = htons(c);
		...
		printf("%ld bytes from %s: ", reply.size, inet_ntoa(rip_addr));
		printf("icmp_seq=%d ", ntohs(icmp_hdr->un.echo.sequence));
...
void traceroute(int sockfd, struct in_addr ip_addr)
{
	...
	do {
		...
		printf(" %d ", c);
		printf(" %s ", inet_ntoa(rip_addr));
		printf(" time=%f ms\n", time_elapsed);

		c++;
	} while(rip_addr.s_addr != ip_addr.s_addr);
}
